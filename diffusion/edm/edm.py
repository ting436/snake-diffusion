...
class EDM(nn.Module):
    ...
    def main_call(self, x: torch.Tensor, sigma: torch.Tensor, prev_frames: torch.Tensor, prev_actions: torch.Tensor):
    ...
    def forward(self, imgs: torch.Tensor, prev_frames: torch.Tensor, prev_actions: torch.Tensor):
        ...
        D_yn = self.main_call(y + n, sigma, prev_frames, prev_actions)
    ...
    def sample(
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
            ...
            # Euler step.
            denoised = self.main_call(x_hat, t_hat, prev_frames, prev_actions).to(torch.float)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < num_steps - 1:
                denoised = self.main_call(x_next, t_next, prev_frames, prev_actions).to(torch.float)
