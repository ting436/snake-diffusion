...
class SnakeGame:
    def __init__(self, w=640, h=480, speed=SPEED, block_size=20):
        self.w = w
        self.h = h
        # init display
    ...
    def _get_direction_from_list_int_value(self, value: List[int]) -> Direction:
        clock_wise = [Direction.RIGHT, Direction.DOWN, Direction.LEFT, Direction.UP]
        idx = clock_wise.index(self.direction)
    ...
    def is_collision(self, point: Optional[Point] = None):
        if point is None:
            point = self.head
        # hits boundary
        if (point.x > self.w - self.block_size or point.x < 0 or \
            point.y > self.h - self.block_size or point.y < 0):
            return True
        # hits itself
